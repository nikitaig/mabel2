...
if ($width) {
    echo '<div class="wopb-builder-container product '.(($has_widget?' wopb-widget-'.$sidebar:'')).'" style="max-width: '.esc_attr($width).'px; margin: 0 auto;">';
    if(is_product()) {
        wc_print_notices();
    }
}
        ...
        if ($page_id) {
            ...
            if (has_blocks($content)) {
                ...
                foreach ( $blocks as $block ) {
                    echo $embed->autoembed(do_shortcode(render_block( $block )));
                }
            } else {
                the_content();
            }
